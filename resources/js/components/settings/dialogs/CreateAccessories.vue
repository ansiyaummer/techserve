<template>
	...
	<v-dialog v-model="dialog" max-width="400">
        <v-card>
            <v-card-title class="subheading grey--text text-uppercase">Add New Accessories</v-card-title>
	            ...
	            <v-card-text>
	                <v-container py-0>
	                    <v-layout wrap>
	                  		...
	                  		<v-flex xs12>
	                    		<v-text-field label="Price" v-model="fields.price" prepend-icon="attach_money" hide-details required></v-text-field>
	                    		<div v-if="errors && errors.price" class="caption red--text font-italic mt-1">{{ errors.price[0] }}</div>
	                  		</v-flex>
		...
		methods: {
		    submit() {
			    if (true) {
			        this.loading = true
			        this.errors = {}
			        axios.post('/storeaccessories', this.fields).then(response => {
			          this.fields = {}
			          this.loading = false
