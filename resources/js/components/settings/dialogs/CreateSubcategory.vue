<template>
	...
	<v-dialog v-model="dialog" max-width="400">
        <v-card>
            <v-card-title class="subheading grey--text text-uppercase">Add New Sub Category</v-card-title>
	            ...
	            <v-card-text>
	                <v-container py-0>
	                    <v-layout wrap>
							...
							<v-flex xs12>
								<v-select :items="roles" label="Choose Category" v-model="fields.role" prepend-icon="check_circle_outline"></v-select>
							</v-flex>
		...
		methods: {
		    submit() {
			    if (true) {
			        this.loading = true
			        this.errors = {}
			        axios.post('/storesubcategory', this.fields).then(response => {
			          this.fields = {}
			          this.loading = false
